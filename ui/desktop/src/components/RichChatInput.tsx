...
const RichTextArea = forwardRef<RichTextAreaRef, RichTextAreaProps>(({
  ...
  const renderContent = useCallback(() => {
    ...
    for (const matchData of processedMatches) {
      ...
      if (type === 'action') {
        ...
        console.log('🏷️ Creating action pill:', { actionLabel, matchedCommand });
        
        if (matchedCommand) {
          parts.push(
            <ActionPill
              key={`action-${keyCounter++}`}
              actionId={matchedCommand.id}
  ...
  }, [value, isFocused, placeholder, cursorPosition, commands]);
...
export default function ChatInput({
  ...
  const { getCommand, expandCommandPrompt, incrementUsage } = useCommands();
  // Add Custom Command Modal state
  const [isAddCommandModalOpen, setIsAddCommandModalOpen] = useState(false);
  const [customCommands, setCommands] = useState<Command[]>([]);
  // Load custom commands on mount
  useEffect(() => {
    const loadCommands = () => {
      try {
        const stored = localStorage.getItem('goose-commands');
        if (stored) {
  ...
  // Handle modal save
  const handleModalSave = (command: Command) => {
    const now = new Date();
    const updatedCommands = [...customCommands, { ...command, createdAt: now, updatedAt: now }];
    try {
      localStorage.setItem('goose-commands', JSON.stringify(updatedCommands));
  ...
  useEffect(() => {
    if (textAreaRef.current && displayValue === "") {
      // Use the RichTextArea resetHeight method if available
  ...
  const expandCommandPills = useCallback((text: string): string => {
